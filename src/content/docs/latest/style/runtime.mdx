...
```html
<div class="md:opacity:hover" style="--opacity-hover: 0.5"></div>
```
...
```html
<div class="dark:opacity:hover" style="--dark-opacity-hover: 0.5"></div>
```

Dark mode can be combined with responsive prefixes and states:

```html
<div class="dark:lg:opacity:hover" style="--dark-lg-opacity-hover: 0.5"></div>
```
...
<span class="z-label z-label-primary">Note</span> Tokens without a state suffix
(e.g. `p=4`) still have their variable injected and their class added, but they
don't go through the interactive CSS generation path — they rely on a rule in
your stylesheet that already references the variable.
