...
<z-lsh
  ...
  toggle
  group="mode"
  cls="z-button z-button-icon z-button-secondary z-button-small relative"
>
...
The automatic cleanup relies on the class prefix. When switching from `z-radius-sharp` to `z-radius-rounded`, the component sees the shared `z-radius-` prefix and smoothly removes the old class before attaching the new one.
