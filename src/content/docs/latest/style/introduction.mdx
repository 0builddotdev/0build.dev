...
|                                        | Tailwind CSS                                                           | 0build                                                      |
| -------------------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------- |
| **Classes**                            | One class per property _per value_ (`m-1`, `m-2`, `m-4`, and so on)    | One class per property (`m`), value supplied separately     |
...
<Alert status="info" icon="info">
  {
    <p>
      Already have a Tailwind layout you like? You don't have to rewrite it by
      hand. Run it through our{" "}
      <a target="_blank" href="https://play.0build.dev/convert">
        converter
      </a>{" "}
      and it'll turn <code class="font-semibold">class="m-8"</code> into{" "}
      <code class="font-semibold">class="m" style="--m: 8"</code> (and
...
<Alert status="info" icon="info">
  {
    <p>
      ...
      has built-in intellisense to spot missing pairs, mismatches, and more. If
      you're working outside the Playground, you can set{" "}
      <code class="font-semibold">zRuntime.debug</code> to{" "}
      <code class="font-semibold">true</code> and open your devtools.
    </p>
  }
</Alert>
...
<Alert status="warning" icon="triangle-alert" title="CLS Alert">
  {
    <p>
      This feature may cause CLS (Cumulative Layout Shift). Make sure to include
      a <code class="font-semibold">z-not-ready</code> class in your{" "}
      <code class="font-semibold">&lt;head&gt;</code> tag.
    </p>
  }
</Alert>
...
```html
<div
  class="content-before bg::before"
  style="--bg-before: var(--color-blue-600)"
></div>
<div
  class="content-before bg:hover::before"
  style="--bg-hover-before: var(--color-blue-800)"
></div>
```
...
<Alert status="warning" icon="triangle-alert" title="CLS Alert">
  {
    <Fragment>
      <p>
        Unlike states or pseudo-classes, pseudo-elements might cause CLS because
        they dynamically inject new content and layout boxes into the DOM on the
        fly, altering element dimensions after the initial paint.
      </p>
      <p class="mt" style="--mt: 2">
        Make sure to include a <code class="font-semibold">z-not-ready</code>{" "}
        class in your <code class="font-semibold">&lt;head&gt;</code> tag.
      </p>
    </Fragment>
  }
</Alert>
...
Starting with `v0.6.4`, shorthand support is now available for [opacity](#opacity).
